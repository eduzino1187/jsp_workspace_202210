...
<%
    /*클라이언트에서 전송된 파라미터를 이용하여 
    오라클에 넣기
    */
    //request 는 내장객체이기 때문에 그 명칭이 이미
    //시스템에 의해 정해져있다.. 
    ...
    //out.print() 는 서버가 클라이언트에 응답할 데이터를 모아놓는
    //출력 스트림이다. print() 메서드에 명시한 문자열은 차곡 차곡 
    //응답객체에 쌓여있다가, 서버가 응답을 할 시점에 다 모아진 
    //문자열을 클라이언트에게 전송한다!!
    if(result >0){
        out.print("<script>");
